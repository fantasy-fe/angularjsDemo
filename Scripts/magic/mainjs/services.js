...
angular.module('bootstrapVariablesEditor.services', []).
  ...
  factory('ap_less', ['$http', function ($http) {
      ...
      var lessEngine = {
          SaveCSSAPI: '/BootstrapSetting/SaveStyle/',
          SaveLessAPI: '/BootstrapSetting/SaveLess/'
      };
      ...
      lessEngine.getUrls = function ($scope) {
          var keys = [
              ...
              "'../img/ap_icons_black_interface-orientation.png'",
              "'../img/ap_icons_white_interface-orientation.png'"
          ];
          return keys;
      };
      ...
      lessEngine.saveLessVar = function (data) {
          ...
          //$http.post(lessEngine.SaveLessAPI, data).success(function (data) {
          //    console.info(data);
          //});

          $.ajax({
              type: "POST",
              url: lessEngine.SaveLessAPI,
              data: { data: data },
              success: function (msg) {
                  //alert("Data Saved: " + msg);
                  $("#downarea").html('<a href="/Content/CustomizedStyle/bootstrap-customized.less" target="_blank">bootstrap-customized.less</a> save successed! ');
                  $("#myDownload").modal('show');
              }
          });

          //var $form = $('<form>').attr('method', 'POST').attr('action', '/Home/SaveLess').append(
          //        $('<input>')
          //            .attr('type', 'hidden')
          //            .attr('name', 'data')
          //            .attr('value', data)
      ...
      lessEngine.saveCSS = function ($scope) {
          ...
          var parser = new (less.Parser)({
              paths: ['/angularjsDemo/Scripts/magic/less/'], // Specify search paths for @import directives
          ...
          $(document).load($('#twitterBootstrapLess').attr('href'), function (data) {
              ...
              parser.parse(data, function (err, tree) {
                  ...
                  //$http.post(lessEngine.SaveCSSAPI, css).success(function (data) {
                  //    console.info(data);
                  //});
                  //$.ajax({
                  //    url: '/Home/SaveStyle/',
                  ...
                  //    }
                  //});
                  $scope.themeModel.Content = css;
                  $scope.themeModel.LessVariable = JSON.stringify($scope.variables);
                  var model = $scope.themeModel;
                  $.ajax({
                      type: "POST",
                      url: lessEngine.SaveCSSAPI,//"/Home/SaveStyle/",
                      data: model,
                      success: function (msg) {
                          $("#downarea").html('<a href="/Content/CustomizedStyle/bootstrap-customized.css" target="_blank">bootstrap-customized.css</a> ');
                          $("#myDownload").modal('show');
                          alert('save success!');
                      }
                  });

                  //$http.post('/Home/SaveStyle/', css)
                  ...
                  //var $form = $('<form>').attr('method', 'POST').attr('action', '/Home/SaveStyle/').
                  //    append(
                  //        $('<input>')
                  //            .attr('type', 'hidden')
                  //            .attr('name', 'data')
